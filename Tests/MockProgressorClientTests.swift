...
final class MockProgressorClientTests: XCTestCase {
    func testRepeatedTareZeroesTheCurrentLoadWithoutAccumulatingOffsets() async throws {
        ...
        try await waitUntil("The mock never reported a connection") { client.state.isConnected }
        client.startStreaming(cause: .initial)
        try await waitUntil("The mock never delivered samples") { samples.count >= 16 }
        ...
        try await waitUntil("The mock never delivered samples after the re-kick") { samples.count > next }
