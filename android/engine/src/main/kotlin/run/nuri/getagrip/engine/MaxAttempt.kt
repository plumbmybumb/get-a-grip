...
/// pull-hold-let-go needs no tap; see `isComplete`. Pure: no clock of its own, no
/// device, no store, which is what makes the rule testable rather than something only
/// verifiable by hanging off a gauge.
///
/// TRANSLATION NOTE (from Shared/Engine/MaxAttempt.swift): a Swift `struct` with
/// `mutating add`/`finish`, held by the caller. Kotlin makes it a class with the same
/// two methods and `private set` on the two published values, which is the twin of
/// Swift's `private(set) var`.
class MaxAttempt {
