...
/// not create another benchmark. Only explicit changes become append-only saves.
struct MaxEditDraft: Equatable, Sendable {
    ...
    mutating func rebase(leftKg: Double?, rightKg: Double?) {
        let leftWasEdited = self.leftKg != (originals[.left] ?? 0)
        let rightWasEdited = self.rightKg != (originals[.right] ?? 0)
    ...
    var hasInvalidChanges: Bool {
        [Side.left, .right].contains { side in
            ...
            return kg != (originals[side] ?? 0) && (!kg.isFinite || kg <= 0)
        }
    }
    ...
    var changes: [Change] {
        [Side.left, .right].compactMap { side in
            let kg = value(for: side)
            guard kg.isFinite, kg > 0, kg != (originals[side] ?? 0) else { return nil }
